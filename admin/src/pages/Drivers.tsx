...
function DriverCard({ driver, onEdit, onClick }: { driver: any; onEdit: () => void; onClick: () => void }) {
    ...
    <div className="flex flex-col gap-5">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4">
            {filtered.map((d: any) => (
              <DriverCard key={d._id} driver={d} onEdit={() => openEdit(d)} onClick={() => navigate(`/drivers/${d._id}`)} />
            ))}
          </div>
        </>
      )}
              ...
              <div className="flex items-center gap-1.5 mt-1.5">
                ...
                {form.status && (
                  <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${
                    STATUS_COLORS[form.status] ?? 'bg-gray-100 text-gray-500 border-gray-200'
                  }`}>
                    {form.status}
                  </span>
                )}
                ...
                return (
                  <button key={s} type="button" onClick={() => set('status', s)}
                    className={`flex flex-col items-center gap-1.5 py-3 px-2 rounded-xl border-2 transition-all ${
                      form.status === s ? colors[s] + ' shadow-sm' : 'border-gray-100 bg-white text-gray-400 hover:border-gray-300'
                    }`}>
                    {icons[s]}
