...
function TripCard({ trip, dispatch, onNavigate }: { trip: any; dispatch: any; onNavigate?: (id: string) => void }) {
...
function TripList({ trips, loading, dispatch, isSuperAdmin, companies, selectedCompanyId, onCompanyChange }: {
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex flex-col gap-2">
            {filtered.map((t: any) => <TripCard key={t._id} trip={t} dispatch={dispatch} onNavigate={(id) => navigate(`/trips/${id}`)} />)}
          </div>
        </>
      )}
    </div>
          ...
          {routes.map((r: any) => (
            <button key={r._id} type="button" onClick={() => set('route', r._id)}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border-2 text-left transition-all ${
                form.route === r._id ? 'border-gray-900 bg-gray-900 text-white' : 'border-gray-100 bg-white hover:border-gray-300'
              }`}>
          ...
          {buses.filter((b: any) => b.status === 'Active').map((b: any) => (
            <button key={b._id} type="button" onClick={() => set('bus', b._id)}
              className={`flex items-center gap-2 px-3 py-2 rounded-xl border-2 text-left transition-all ${
                form.bus === b._id ? 'border-gray-900 bg-gray-900 text-white' : 'border-gray-100 bg-white hover:border-gray-300'
              }`}>
              <div className={`w-6 h-6 rounded-md flex items-center justify-center shrink-0 ${
                form.bus === b._id ? 'bg-white/15' : 'bg-gray-100'
              }`}>
...
export default function Trips() {
  ...
  useEffect(() => {
    ...
    dispatch(fetchRoutes({ company: companyFilter }));
  ...
  const goToDay = (dateKey: string) => {
    ...
    if (blockedDates.includes(dateKey)) { dispatch(toggleBlockedDate(dateKey)); return; }
            ...
            <>
              ...
              <Card className="p-0">
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                  ...
                  {!isBlocked && <Button onClick={() => openCreate(selectedDate)}>+ Add Trip</Button>}
                </div>
                ...
                {isBlocked ? (
                  ...
                  <TripList trips={dayTrips} loading={loading} dispatch={dispatch} />
                )}
              </Card>
            </>
          )}
