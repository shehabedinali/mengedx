  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={`ml-auto text-[11px] font-semibold px-2.5 py-1 rounded-full ${hasDriver ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-amber-50 text-amber-600 border border-amber-200'
          }`}>
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {!isDispatcher && <Button onClick={() => setOpen(true)}>+ Add Bus</Button>}
        </div>
      </div>
                ...
                {BUS_STATUSES.map(s => {
                  ...
                  return (
                    <button key={s} type="button" onClick={() => set('status', s)}
                      className={`py-2 rounded-xl border-2 text-xs font-bold transition-all ${form.status === s ? statusColors[s] + ' shadow-sm' : 'border-gray-100 bg-white text-gray-400 hover:border-gray-300'
                        }`}>
                      {s}
                    </button>
                  );
